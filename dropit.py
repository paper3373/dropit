...
class DropITer(object):
    def __init__(self, strategy, gamma, d=16384):
        self.gamma = gamma
        ...
        self.double_nsigma = torch.sqrt(torch.tensor(2)) * torch.erfinv(torch.tensor(gamma))
    ...
    def select_nsigma(self, x: torch.Tensor):
        if hasattr(x, "idxs"):
            ...
            t1, t2 = mean - self.double_nsigma * std, mean + self.double_nsigma * std
