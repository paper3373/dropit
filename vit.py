...
class Mlp(nn.Module):
    ...
    def get_threshold(self, x, gamma):
        if not hasattr(self, "nsigma"):
            self.nsigma = torch.sqrt(torch.tensor(2)) * torch.erfinv(2 * torch.tensor(gamma) - 1)
        with torch.no_grad():
            mean_ = x.mean()
            std_ = x.std(unbiased=False)
            threshold = mean_ + self.nsigma * std_
        return threshold

    def forward(self, x):
        x = self.fc1(x)
        if self.training and hasattr(self.fc2, "dropiter"):
            threshold = self.get_threshold(x, self.fc2.dropiter.gamma)
            idxs = (x.view(-1) > threshold).nonzero(as_tuple=True)[0]
        x = self.act(x)
        x = self.drop1(x)
        if self.training and hasattr(self.fc2, "dropiter"):
            x.idxs = idxs
